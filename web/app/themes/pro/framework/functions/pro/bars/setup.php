...
function x_bars_setup_modules() {
  ...
  if ( ! is_null( $header_data ) || ! is_null( $footer_data ) ) {
    add_action( 'x_bar', 'x_render_elements', 10, 2 );
    add_action( 'x_bar_container', 'x_render_elements', 10, 2 );
  }
  ...
  if ( ! is_null( $header_data ) ) {
    ...
    x_action_defer( 'x_masthead', 'x_render_elements', array( $header_data['modules'] ), 10, true );
  ...
  if ( ! is_null( $footer_data ) ) {
    ...
    x_action_defer( 'x_colophon', 'x_render_elements', array( $footer_data['modules'] ), 10, true );
...
function x_bars_setup_header_spaces( $header_data ) {
  ...
  foreach ( $header_data['modules'] as $bar ) {
    if ( isset( $bar_space_actions[ $bar['_region']] ) ) {
      unset( $bar['_modules'] );
      x_set_view( $bar_space_actions[ $bar['_region'] ], 'elements', 'bar', 'space', x_element_decorate( $bar ) );
    }
  }
...
function x_bars_element_setup() {
  require_once( X_TEMPLATE_PATH . '/framework/functions/pro/bars/definitions/bar.php' );
  require_once( X_TEMPLATE_PATH . '/framework/functions/pro/bars/definitions/container.php' );
  require_once( X_TEMPLATE_PATH . '/framework/functions/pro/bars/definitions/layout-grid.php' );
  require_once( X_TEMPLATE_PATH . '/framework/functions/pro/bars/definitions/layout-cell.php' );
}
